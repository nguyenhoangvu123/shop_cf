...
const supplies = {
    ...
    list: (page = 1) => {
        $.ajax({
            type: 'POST',
            url: route('admin.accounting.attr.supplies.list'),
            data: {
                page: page
            },
    ...
    save: () => {
        ...
        if (createContructionForm.valid()) {
            ...
            $.ajax({
                type: 'POST',
                url: route('admin.accounting.attr.supplies.store'),
                data: data,
                success: function (data) {
                    if (!data.error) {
                        Swal.fire(
                            ...
                            if (result.isConfirmed) {
                                window.location.replace(route('admin.accounting.attr.supplies'))
                            }
                        });
                    }
                },
    ...
    update: (id) => {
        ...
        if (updateContructionForm.valid()) {
            ...
            $.ajax({
                type: 'POST',
                url: route('admin.accounting.attr.supplies.update', { id }),
                data: data,
                success: function (data) {
                    if (!data.error) {
                        Swal.fire(
    ...
    deleteContruction: (id) => {
        Swal.fire({
            title: 'Bạn muốn xoá gói vật tư hoàn thiện  ?',
            text: "Xoá xoá gói vật tư hoàn thiện sẽ không thể khôi phục lại !",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'DELETE',
                    url: route('admin.accounting.attr.supplies.delete', { id }),
                    data: {
                    },
                    success: function (data) {
                        if (!data.error) {
